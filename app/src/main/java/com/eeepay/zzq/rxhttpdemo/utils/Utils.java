...
//            if (activity.getClass() == PermissionUtils.PermissionActivity.class) return;
    ...
    public static Double getMaxValue(List<String> list) {
        ...
        for (String item : list) {
            Double doubleValue = TextUtils.isEmpty(item) ? 0.00D : Double.parseDouble(item);
            maxId = doubleValue > maxId ? doubleValue : maxId;
        }
        return maxId;
    }

    // 获取最大值
    public static Double getMaxValue(String[] list) {
        Double maxId = 0.00D;
        for (String item : list) {
            Double doubleValue = TextUtils.isEmpty(item) ? 0.00D : Double.parseDouble(item);
            maxId = doubleValue > maxId ? doubleValue : maxId;
